...
QUnit.module('Тестируем функцию sort', function () {
	...
	QUnit.test('Проверка работы с некорректными данными', function(assert) {
		assert.strictEqual(sort(1), '', 'Number');
		assert.strictEqual(sort(new Object()), '', 'Object');
		assert.strictEqual(sort(true), '', 'Boolean');
		assert.strictEqual(sort(function (){}), '', 'Function');
		assert.strictEqual(sort([]),'', 'Array');
		assert.strictEqual(sort(new Date()),'', 'Date');
